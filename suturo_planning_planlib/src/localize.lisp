...
(defparameter *gap-box-center-robot* 0.8)

(defun init-localize ()
  (setf *location-on-table-nr* 0)
  (setf *location-on-counter-nr* 0)
  (setf *location-on-red-box-nr* 0)
  (setf *locations-on-table* nil)
  (setf *locations-on-counter* nil)
       ...
       ((desig-prop-value loc 'loc)
        (let* ((loc2 (desig-prop-value loc 'loc))
               ...
               (x (if (equal (desig-prop-value loc2 'name) *counter-name*)
                      ...
                      (if (equal (desig-prop-value loc2 'name) *table-name*)
                          (+ (cl-tf:x origin) *gap-table-center-robot*)
                          (+ (cl-tf:x origin) *gap-box-center-robot*))))
  ...
  (let* ((obj (get-furniture name))
    ...
    (cond 
      ...
      ((equal name *red-box-name*)
       ...
       (push (make-pose `(,(+ x gripper-offset) ,(- y position-over-diff) ,(- z 0.05)) *quaternion-table*) *locations-on-red-box*)
       (push (make-pose `(,(+ x gripper-offset) ,y ,(- z 0.05)) *quaternion-table*) *locations-on-red-box*)
       (push (make-pose `(,(+ x gripper-offset) ,(+ y position-over-diff) ,(+ z 0.05)) *quaternion-table*) *locations-on-red-box*)
       (push (make-pose `(,(+ x gripper-offset) ,(- y position-over-diff) ,(+ z 0.05)) *quaternion-table*) *locations-on-red-box*)
       (push (make-pose `(,(+ x gripper-offset) ,y ,(+ z 0.05)) *quaternion-table*) *locations-on-red-box*)
       (push (make-pose `(,(+ x gripper-offset) ,(+ y position-over-diff) ,z) *quaternion-table*) *locations-on-red-box*)
       (push (make-pose `(,(+ x gripper-offset) ,(- y position-over-diff) ,z) *quaternion-table*) *locations-on-red-box*)
       (push (make-pose `(,(+ x gripper-offset) ,y ,z) *quaternion-table*) *locations-on-red-box*)))))
