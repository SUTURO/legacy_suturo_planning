  ...
  (let ((objs (subseq (first (first jj)) 1)))
    (mapcar (lambda (obj)
              (let* ((edible (first obj))
                    ...
                    (centroid (fourth obj))
                    (frame-id (fifth obj))
                    (grip-force (sixth obj))
                    (use (seventh obj))
                    (loc (make-designator 'location `((coords ,centroid) (frame ,(symbol->string frame-id))))))
                (make-designator 'object `((type ,(intern (nstring-upcase (symbol->string typ))))
